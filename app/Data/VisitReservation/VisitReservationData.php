<?php

namespace App\Data\VisitReservation;

use App\Data\VisitAttendee\VisitAttendeeData;
use App\Data\VisitSlot\VisitSlotData;
use App\Data\Visitor\VisitorData;
use App\Models\VisitReservation;
use Spatie\LaravelData\Data;
use Spatie\LaravelData\Lazy;

/**
 * Output DTO for one booking.
 *
 * The visitor's name, email and phone are FLATTENED onto the row as well as being
 * available through the lazy relation, because the index renders all three in one
 * column and eager-loading the relation only to reach into it would make the list
 * page depend on an include the drawer needs and the table does not — the same
 * shape JobApplicationData uses for its candidate.
 *
 * `attendees` is Lazy on its own relation rather than on the reservation, so the
 * index can eager-load the visitor for the flat scalars without dragging every
 * child row along with it.
 */
class VisitReservationData extends Data
{
    public function __construct(
        public string $id,
        public string $visit_slot_id,
        public string $visit_service_id,
        public string $visitor_id,
        public ?string $form_submission_id,
        public int $visitors_count,
        public string $status,
        public ?string $booked_at,
        public ?string $note,
        public ?string $visitor_name,
        public ?string $visitor_email,
        public ?string $visitor_phone,
        public ?string $service_name,
        public ?string $slot_starts_at,
        public ?string $slot_ends_at,
        public ?string $created_at,
        public ?string $updated_at,
        public Lazy|VisitorData|null $visitor,
        public Lazy|VisitSlotData|null $slot,
        /** @var Lazy|array<int, VisitAttendeeData> */
        public Lazy|array $attendees,
    ) {}

    public static function fromModel(VisitReservation $reservation): self
    {
        $visitor = $reservation->relationLoaded('visitor') ? $reservation->visitor : null;
        $slot = $reservation->relationLoaded('slot') ? $reservation->slot : null;
        $service = $reservation->relationLoaded('service') ? $reservation->service : null;

        return new self(
            id: $reservation->id,
            visit_slot_id: $reservation->visit_slot_id,
            visit_service_id: $reservation->visit_service_id,
            visitor_id: $reservation->visitor_id,
            form_submission_id: $reservation->form_submission_id,
            visitors_count: (int) $reservation->visitors_count,
            status: $reservation->status->getValue(),
            booked_at: $reservation->booked_at?->toIso8601String(),
            note: $reservation->note,
            visitor_name: $visitor?->full_name,
            visitor_email: $visitor?->email,
            visitor_phone: $visitor?->phone,
            service_name: $service?->name,
            slot_starts_at: $slot?->starts_at?->toIso8601String(),
            slot_ends_at: $slot?->ends_at?->toIso8601String(),
            created_at: $reservation->created_at?->toIso8601String(),
            updated_at: $reservation->updated_at?->toIso8601String(),
            visitor: Lazy::whenLoaded('visitor', $reservation, fn () => $reservation->visitor ? VisitorData::from($reservation->visitor) : null),
            slot: Lazy::whenLoaded('slot', $reservation, fn () => $reservation->slot ? VisitSlotData::from($reservation->slot) : null),
            attendees: Lazy::whenLoaded('attendees', $reservation, fn () => VisitAttendeeData::collect($reservation->attendees)),
        );
    }
}
