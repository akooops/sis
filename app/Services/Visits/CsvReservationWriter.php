...
/**
 * The visit reservations CSV — the register the front desk prints.
 *
...
class CsvReservationWriter
{
    ...
    public function stream(EloquentBuilder $query): StreamedResponse
    {
        ...
        $callback = function () use ($query, $chunk) {
             ...
             */
            foreach ($query->with(['visitor', 'service', 'slot', 'attendees'])->reorder()->lazyById($chunk) as $reservation) {
     ...
     */
    protected function row(VisitReservation $reservation): array
    {
        $visitor = $reservation->visitor;

        $students = $reservation->attendees
            ->map(fn ($attendee) => trim($attendee->full_name.' ('.($attendee->grade ?: '?').')'))
            ->implode('; ');
        ...
        return array_map([$this, 'safe'], [
            ...
            (string) $reservation->visitors_count,
            $students,
    ...
    {
        return 'visit-reservations-'.now()->format('Ymd-His').'.csv';
    }
}
