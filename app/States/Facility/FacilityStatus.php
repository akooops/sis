...
/**
 * Where a facility sits editorially. Only Published is served.
 *
 ...
 *
 * The same shape and rationale as every other publishable module here: Published
 * is reachable from everywhere so a venue can always go back up, Hidden from
 * anything that was public so it can always come down, Published -> Draft is
 * barred (taking a live page down is an explicit Hidden, worth its own audit row)
 * and so is Draft -> Hidden (a draft was never public).
 *
 * This says nothing about whether anyone can BOOK: that is derived from the
 * venue's slots, so hiding one stops new bookings without touching the
 * reservations already made against it.
 */
abstract class FacilityStatus extends State
{
