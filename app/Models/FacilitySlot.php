...
/**
 * One offered time on one venue, with a limit. The twin of VisitSlot.
 *
 * CAPACITY COUNTS RESERVATIONS. A booking takes one seat — unlike a visit, a
 * venue booking has no party size, so there is no second number to confuse this
 * one with.
 *
...
class FacilitySlot extends Model
{
    ...
    public function reservedCount(): int
    {
        if ($this->relationLoaded('reservations')) {
            return $this->reservations
                ->reject(fn (FacilityReservation $reservation) => $reservation->status instanceof Cancelled)
                ->count();
        }
    ...
    public function state(): string
    {
        ...
        return $remaining <= (int) config('facilities.limited_at', 2) ? 'limited' : 'open';
    }
