<script>
     ...
     * is fetched whole from api.v1.admin.facility-slots.calendar each time the view
     ...
     *
     * ONE FACILITY AT A TIME. Slots belong to a facility and their limits mean nothing
     * pooled together, so the picker is required rather than being an "all" filter.
     * It reads ?filter[facility_id]= so the Time slots action on the services
    ...
    import { SLOT_STATE_CLASSES, SLOT_STATE_LABELS } from '@/lib/facility';

    /** The service the page opened on, from the deep link. */
    const initialServiceId = new URLSearchParams(window.location.search).get('filter[facility_id]');
    ...
    const canWrite = hasPermission('facility-slots.store');

    /*
     * The picker's own options. Loaded once rather than through a resource-select,
     * because the page also needs the CHOSEN facility's default limit
     * to pre-fill the drawers, and a remote select hands back only an id.
     */
    $effect(() => {
        api.get(route('api.v1.admin.facilities.index'), { per_page: 100, sort: 'order' })
            .then((d) => {
                services = d?.data ?? [];
    ...
    /*
     * Refetch when the facility changes. `calendar` is read so the effect waits for it
    ...
    async function load(info, success, failure) {
        ...
        try {
            const rows = await api.get(route('api.v1.admin.facility-slots.calendar'), {
                facility_id: serviceId,
                from: info.startStr,
                to: info.endStr,
            });
...
<svelte:head><title>Saud International Schools — Facility time slots</title></svelte:head>

<AdminLayout title="Facility time slots" breadcrumbs={[{ label: 'Facilities' }, { label: 'Time slots' }]}>
    <Card>
        ...
        <div class="p-5">
            ...
            {#if services.length === 0}
                <EmptyState
                    ...
                    body="Add a facility before giving it times."
                />
            {:else}
