...
<form class="flex w-full flex-col gap-5" onsubmit={submit}>
    ...
    {#if !editing || activeTab === 'details'}
        <div class="flex flex-col gap-5">
            ...
            <Field label="Stylesheet URL" error={form.errors.css_url} hint="Optional. A hosted stylesheet for this achievement's content HTML only. Scope every selector to #page-content — the id on the content container — so nothing leaks into the rest of the page.">
                <Input bind:value={form.data.css_url} invalid={!!form.errors.css_url} placeholder="https://…" />
            </Field>

            <Field label="Custom CSS" error={form.errors.custom_css} hint="Inline CSS for this achievement's content HTML only, loaded after the stylesheet above so rules here win. Scope every selector to #page-content — the id on the content container — so nothing leaks into the rest of the page.">
                <textarea
