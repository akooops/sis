<script>
    ...
    async function load(id) {
        ...
        try {
            record = await api.get(route('api.v1.admin.visit-reservations.show', id));
            note = record?.note ?? '';
        } catch (err) {
    ...
    async function transition(action) {
        ...
        try {
            record = await api.post(route(`api.v1.admin.visit-reservations.${action}`, record.id));
    ...
    async function saveNote() {
        ...
        try {
            record = await api.put(route('api.v1.admin.visit-reservations.update', record.id), { note });
    ...
    {:else}
        <div class="flex flex-col gap-6">
            <div class="flex flex-wrap items-center gap-2">
                <Badge variant={VISIT_RESERVATION_STATUS_VARIANTS[record.status] ?? 'secondary'}>
                    {VISIT_RESERVATION_STATUS_LABELS[record.status] ?? record.status}
                </Badge>
            ...
            <div class="flex flex-col gap-2">
                ...
                <dl class="flex flex-col divide-y divide-border rounded-lg border border-border text-sm">
                    ...
                    <div class="flex items-center justify-between gap-3 px-3 py-2">
                        <dt class="shrink-0 text-muted-foreground">Visitors</dt>
                        <dd class="text-end text-mono">{record.visitors_count}</dd>
                    </div>
                    <div class="flex items-center justify-between gap-3 px-3 py-2">
            ...
            <VisitorProfile visitor={record.visitor} attendees={record.attendees ?? []} />

            {#if hasPermission('visit-reservations.update')}
                <Field label="Internal note" hint="The desk's own. Never shown to the family.">
