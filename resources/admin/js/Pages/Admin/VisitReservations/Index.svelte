<script>
    ...
    import {
        VISIT_RESERVATION_STATUS_LABELS,
        VISIT_RESERVATION_STATUS_VARIANTS,
        availableTransitions,
    } from '@/lib/visitReservation';
    import { hasPermission } from '@/lib/permissions';
    import { api } from '@/lib/api/client';
    import { toast } from '@/lib/toast';
    import { confirm } from '@/lib/confirm';

    const list = useIndex('api.v1.admin.visit-reservations.index', { perPage: 15, sort: '-booked_at' });
    ...
    const serviceId = $derived(list.params.filter?.visit_service_id ?? null);
    ...
    const filteredService = $derived(
        serviceId ? (list.rows.find((r) => r.visit_service_id === serviceId)?.service_name ?? null) : null,
    );
    ...
    const columns = $derived([
        ...
        { key: 'visitor', label: 'Guardian', truncate: false },
        serviceId ? null : { key: 'service_name', label: 'Visit', truncate: false },
    ...
    const filterConfig = [
        {
            key: 'visit_service_id',
            type: 'resource-select',
            label: 'Visit',
            resource: 'api.v1.admin.visit-services.index',
        ...
        },
        {
            ...
            type: 'select',
            label: 'Status',
            options: Object.entries(VISIT_RESERVATION_STATUS_LABELS).map(([value, label]) => ({ value, label })),
        },
    ...
    const canExport = hasPermission('visit-reservations.export');
    ...
    async function transition(row, action) {
        try {
            await api.post(route(`api.v1.admin.visit-reservations.${action}`, row.id));
    ...
    async function remove(row) {
        ...
        try {
            await api.delete(route('api.v1.admin.visit-reservations.destroy', row.id));
...
<svelte:head><title>Saud International Schools — Visit bookings</title></svelte:head>
...
{#snippet toolbar()}
    ...
    {#if canExport}
        <ServerExportButton
            routeName="api.v1.admin.visit-reservations.export"
            params={exportParams}
            fallbackFilename="visit-reservations.csv"
            label="Export CSV"
...
{#snippet table()}
    <DataTable
        ...
        emptyBody="Bookings made on the public visits page land here."
        {cells}
        {rowActions}
    />
{/snippet}
...
{#snippet cells(row, column)}
    ...
    {:else if column.key === 'status'}
        <Badge variant={VISIT_RESERVATION_STATUS_VARIANTS[row.status] ?? 'secondary'}>
            {VISIT_RESERVATION_STATUS_LABELS[row.status] ?? row.status}
        </Badge>
...
{#snippet rowActions(row)}
    <RowActions actions={[
        ...
        hasPermission('visit-reservations.destroy') && { icon: 'ki-trash', label: 'Delete', onclick: () => remove(row), variant: 'destructive' },
    ].filter(Boolean)} />
{/snippet}
