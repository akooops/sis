<script>
     ...
     * moving a slot between visits would carry its bookings with it and silently
    ...
    import { SLOT_STATE_LABELS, SLOT_STATE_VARIANTS, slotOccupancy } from '@/lib/visitService';
    ...
    async function submit(event) {
        ...
        const url = editing
            ? route('api.v1.admin.visit-slots.update', visitSlot.id)
            : route('api.v1.admin.visit-slots.store');
        ...
        try {
            const res = await form.submit(editing ? 'put' : 'post', url, {
                transform: (data) => ({
                    ...
                    ...(editing ? {} : { visit_service_id: serviceId }),
                }),
            });
    ...
    async function remove() {
        ...
        try {
            await api.delete(route('api.v1.admin.visit-slots.destroy', visitSlot.id));
...
{#snippet footer()}
    {#if editing && hasPermission('visit-slots.destroy')}
