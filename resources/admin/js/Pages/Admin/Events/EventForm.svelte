<script>
     ...
     * there is nothing to translate until the event exists. Edit splits into
    ...
    import { EVENT_STATUS_LABELS, needsPublishedAt, reachableStatuses } from '@/lib/event';

    let { event = null, ready = true, onsaved, oncancel } = $props();
    ...
    const form = useForm(
        event
            ? {
                  ...
                  published_at: event.published_at ? event.published_at.slice(0, 16).replace('T', ' ') : null,
                  // When the event runs — nothing to do with when it goes live.
                  start_at: event.start_at ? event.start_at.slice(0, 16).replace('T', ' ') : null,
                  end_at: event.end_at ? event.end_at.slice(0, 16).replace('T', ' ') : null,
    ...
    const statusOptions = $derived(
        reachableStatuses(event?.status ?? null).map((value) => ({ value, label: EVENT_STATUS_LABELS[value] ?? value })),
    );
    ...
    // `e`, not `event`: the record prop is called `event`, and naming the DOM
    // event the same thing shadows it — route(…, event.id) then resolves to the
    // submit event's id, which is undefined.
    async function submit(e) {
        e.preventDefault();
        const url = editing ? route('api.v1.admin.events.update', event.id) : route('api.v1.admin.events.store');
        try {
...
<form class="flex w-full flex-col gap-5" onsubmit={submit}>
    ...
    {#if !editing || activeTab === 'details'}
        <div class="flex flex-col gap-5">
            <Field label="Thumbnail" error={form.errors.thumbnail} required={!editing} hint="Shown wherever the event is listed.">
                <MediaPicker accept={['images']} bind:value={form.data.thumbnail} previewUrl={event?.thumbnail_url} />
            </Field>
            ...
            <!-- The event's own dates, kept apart from the publish schedule
                 above: an event can be published long before it runs, and a past
                 event can still be a live listing. -->
            <div class="grid grid-cols-1 gap-5 sm:grid-cols-2">
                <Field label="Starts at" error={form.errors.start_at} required hint="When the event itself begins.">
                    <DatePicker enableTime bind:value={form.data.start_at} invalid={!!form.errors.start_at} />
                </Field>
                <Field label="Ends at" error={form.errors.end_at} required hint="Must be on or after the start.">
                    <DatePicker enableTime bind:value={form.data.end_at} invalid={!!form.errors.end_at} />
                </Field>
            </div>

            <Field label="Stylesheet URL" error={form.errors.css_url} hint="Optional. A hosted stylesheet for this event's content HTML only. Scope every selector to #page-content — the id on the content container — so nothing leaks into the rest of the page.">
                <Input bind:value={form.data.css_url} invalid={!!form.errors.css_url} placeholder="https://…" />
            </Field>

            <Field label="Custom CSS" error={form.errors.custom_css} hint="Inline CSS for this event's content HTML only, loaded after the stylesheet above so rules here win. Scope every selector to #page-content — the id on the content container — so nothing leaks into the rest of the page.">
                <textarea
