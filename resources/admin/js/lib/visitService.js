/**
 * Visit service status — mirrors App\States\VisitService\VisitServiceStatus. Only
 * `published` is served.
 *
 ...
 * service's slots, so a published visit with no upcoming times still shows on the
 * site and says it has none.
 */
export const VISIT_SERVICE_STATUS_LABELS = {
    draft: 'Draft',
    scheduled: 'Scheduled',
    published: 'Published',
    hidden: 'Hidden',
...
export const VISIT_SERVICE_STATUS_VARIANTS = {
    draft: 'secondary',
    scheduled: 'warning',
    published: 'success',
    hidden: 'destructive',
...
/**
 * Mirrors VisitServiceStatus::config(). The 422 stays the enforcement — this only
 * stops the form offering a door the server has locked.
 */
export const VISIT_SERVICE_STATUS_TRANSITIONS = {
...
export function reachableStatuses(current = null) {
    ...
    return [current, ...(VISIT_SERVICE_STATUS_TRANSITIONS[current] ?? [])];
}
...
/**
 * Mirrors VisitSlot::STATES, and VisitSlot::state() is what decides which one a
