/**
 * Brand status — mirrors App\States\Brand\BrandStatus. Only `published` is served.
 *
 * `scheduled` is a promise the server keeps: brands:publish-scheduled flips it
...
/**
 * Mirrors BrandStatus::config(). The 422 stays the enforcement — this only stops
 ...
 * to Draft (pulling a live brand is an explicit take-down, via Hidden).
 */
...
export function reachableStatuses(current = null) {
    ...
    return [current, ...(BRAND_STATUS_TRANSITIONS[current] ?? [])];
}
