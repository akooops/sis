/**
 * Article status — mirrors App\States\Article\ArticleStatus. Only `published` is served.
 *
 * `scheduled` is a promise the server keeps: articles:publish-scheduled flips it
...
export const ARTICLE_STATUS_VARIANTS = {
    draft: 'secondary',
    scheduled: 'warning',
    published: 'success',
    hidden: 'destructive',
...
/**
 * Mirrors ArticleStatus::config(). The 422 stays the enforcement — this only stops
 ...
 * to Draft (pulling a live article is an explicit take-down, via Hidden).
 */
...
export function reachableStatuses(current = null) {
    ...
    return [current, ...(ARTICLE_STATUS_TRANSITIONS[current] ?? [])];
}
