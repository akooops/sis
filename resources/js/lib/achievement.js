/**
 * Achievement status — mirrors App\States\Achievement\AchievementStatus. Only `published` is served.
 *
 * `scheduled` is a promise the server keeps: achievements:publish-scheduled flips it
...
export const ACHIEVEMENT_STATUS_VARIANTS = {
    draft: 'secondary',
    scheduled: 'warning',
    published: 'success',
    hidden: 'destructive',
...
/**
 * Mirrors AchievementStatus::config(). The 422 stays the enforcement — this only stops
 ...
 * to Draft (pulling a live achievement is an explicit take-down, via Hidden).
 */
export const ACHIEVEMENT_STATUS_TRANSITIONS = {
...
export function reachableStatuses(current = null) {
    ...
    return [current, ...(ACHIEVEMENT_STATUS_TRANSITIONS[current] ?? [])];
}
