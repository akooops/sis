<script>
    ...
    const breadcrumbs = [
        ...
        {
            title: 'Edit',
            url: route('admin.programs.edit', { program: program?.id }),
            active: true
        }
    ];
    
    const pageTitle = 'Edit Program';

    // Form data for basic program info
    let form = {
        name: program?.name || '',
        slug: program?.slug || '',
        media_option: 'upload',
    ...
    if (languages && Array.isArray(languages)) {
        languages.forEach(language => {
            ...
            const subtitle = translations?.subtitle?.[language.code] || '';
    ...
    function handleSubmit() {
        ...
        router.post(route('admin.programs.update', { program: program.id }), formData, {
            onError: (err) => {
                errors = err;
                loading = false;
    ...
    function handleTranslationSubmit(languageCode, languageId) {
        ...
        formData.append('subtitle', translationForms[languageCode].subtitle);
        ...
        // Send AJAX request
        fetch(route('admin.programs.update-translation', { program: program.id }), {
            method: 'POST',
            body: formData,
            headers: {
        ...
        <div class="grid gap-5 lg:gap-7.5">
            ...
            <div class="flex flex-col lg:flex-row lg:items-center lg:justify-between gap-4">
                <div class="flex flex-col gap-1">
                    <h1 class="text-2xl font-bold text-mono">Edit Program</h1>
                    <p class="text-sm text-secondary-foreground">
                        Update program information and content
                    </p>
                </div>
            ...
            <div class="kt-card w-full">
                <div class="kt-card-content">
                    ...
                    <div class="kt-tabs kt-tabs-line justify-between mb-6" data-kt-tabs="true">
                        <div class="flex items-center gap-5">
                            <button 
                                ...
                                data-kt-tab-toggle="#program_form_tab"
                            >
                    ...
                    <!-- Tab Content -->
                    <!-- Program Form Tab -->
                    <div class="grow flex flex-col" id="program_form_tab">
                                    ...
                                    <div class="grid gap-4">
                                        ...
                                        <div class="flex flex-col gap-2">
                                            ...
                                            <input
                                                ...
                                                disabled={program?.is_system_program}
                                            />
                                            {#if program?.is_system_program}
                                                <p class="text-sm text-muted-foreground">System programs cannot have their slug changed</p>
                                            {/if}
                                            {#if errors.slug}
                            ...
                            {#each languages as language, index}
                                ...
                                >
                                    ...
                                    >
                                        ...
                                        <!-- Program Subtitle -->
                                        <div class="flex flex-col gap-2">
                                            <label class="text-sm font-medium text-mono" for="subtitle-{language.id}">
                                                Subtitle <span class="text-destructive">*</span>
                                            </label>
                                            <input
                                                id="subtitle-{language.id}"
                                                type="text"
                                                class="kt-input {translationErrors[language.code]?.subtitle ? 'kt-input-error' : ''}"
                                                placeholder="Enter program subtitle"
                                                bind:value={translationForms[language.code].subtitle}
                                            />
                                            {#if translationErrors[language.code]?.subtitle}
                                                <p class="text-sm text-destructive">{translationErrors[language.code].subtitle[0]}</p>
                                            {/if}
                                        </div>
