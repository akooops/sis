<script>
    ...
    async function handleSubmit() {
        try {
            ...
            let url;
            if (isEditMode) {
                url = route('admin.facility-time-slots.update', editingTimeSlot.id);
            } else if (isBulkCreate) {
                url = route('admin.facility-time-slots.bulk-store', facility.id);
            } else {
                url = route('admin.facility-time-slots.store', facility.id);
            }
    ...
    async function deleteTimeSlot(timeSlot) {
        ...
        try {
            ...
            const response = await fetch(route('admin.facility-time-slots.destroy', { facilityTimeSlot: timeSlot.id }), {
                method: 'POST',
                headers: {
                    'X-Requested-With': 'XMLHttpRequest'
                },
    ...
    function fetchTimeSlots(min, max) {
        fetch(route('admin.facility-time-slots.index', facility.id) + `?min=${min}&max=${max}`, {
            headers: {
                'X-Requested-With': 'XMLHttpRequest'
            }
        })
        ...
        .then(data => {
            if (window.calendar) {
                ...
                data.facilityTimeSlots.forEach(slot => {
        ...
        <div class="grid gap-5 lg:gap-7.5">
            ...
            <div class="flex flex-col lg:flex-row lg:items-center lg:justify-between gap-4">
                <div class="flex flex-col gap-1">
                    <h1 class="text-2xl font-bold text-mono">Time Slots</h1>
                    <p class="text-sm text-secondary-foreground">
                        Manage time slots for "{facility?.name}"
                    </p>
                </div>
                <div class="flex items-center gap-3">
                    {#if hasPermission('admin.facility-time-slots.store')}
