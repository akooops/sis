<script>
    ...
    $: if (productionSite) {
        form = {
            ...
            logo: null,
            code: productionSite.code || '',
            description: productionSite.description || '',
            address: productionSite.address || '',
            city: productionSite.city || '',
            state: productionSite.state || '',
            zip: productionSite.zip || '',
            country: productionSite.country || '',
            website: productionSite.website || '',
            contact_person: productionSite.contact_person || '',
            contact_person_email: productionSite.contact_person_email || '',
            contact_person_phone: productionSite.contact_person_phone || '',
            contact_person_position: productionSite.contact_person_position || '',
            contact_person_department: productionSite.contact_person_department || '',
            contact_person_department_email: productionSite.contact_person_department_email || '',
            contact_person_department_phone: productionSite.contact_person_department_phone || '',
            contact_person_department_position: productionSite.contact_person_department_position || '',
        };
    }
    ...
    async function handleSubmit() {
        ...
        try {
            ...
            const response = await fetch(route('api.v1.admin.production-sites.update', productionSite.id), {
                method: 'POST',
                headers: {
            ...
            if (response.ok) {
                toast('Production site updated successfully', 'success');
                dispatch('updated');
            } else {
                if (data.errors) {
                    errors = data.errors;
                } else {
                    console.error('Error updating production site:', data.message || 'Unknown error');
                }
            }
        } catch (error) {
    ...
    <form on:submit|preventDefault={handleSubmit} class="space-y-5">
        ...
        <div class="kt-card border border-border">
            <div class="kt-card-header border-b border-border">
                <span class="text-sm font-semibold text-mono">Production Site</span>
            </div>
            <div class="kt-card-content p-4 space-y-5">
                <div class="flex flex-col gap-2">
                    <ImageInput
                        bind:value={form.logo}
                        defaultImage={productionSite?.logo_url}
                        disabled={loading}
