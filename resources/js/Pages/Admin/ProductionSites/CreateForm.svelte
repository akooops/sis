<script>
    ...
    async function handleSubmit() {
        ...
        try {
            ...
            const response = await fetch(route('api.v1.admin.production-sites.store'), {
                method: 'POST',
                headers: {
            ...
            if (response.ok) {
                ...
                toast('Production site created successfully', 'success');
                dispatch('created');
            } else {
                if (data.errors) {
                    errors = data.errors;
                } else {
                    console.error('Error creating production site:', data.message || 'Unknown error');
                }
            }
        } catch (error) {
