<script>
    ...
    export let productionProcess = null;
    ...
    $: if (productionProcess) {
        form = {
            ...
            order: productionProcess.order ?? 0,
        };
    }
    ...
    async function handleSubmit() {
        ...
        try {
            ...
            const response = await fetch(route('api.v1.admin.production-processes.update', { productionProcess: productionProcess.id }), {
                method: 'POST',
                headers: {
            ...
            if (response.ok) {
                toast('Production process updated successfully', 'success');
                dispatch('updated');
            } else if (data.errors) {
                errors = data.errors;
            } else {
                errors = { general: data.message || 'An error occurred while updating the production process.' };
            }
        } catch (error) {
