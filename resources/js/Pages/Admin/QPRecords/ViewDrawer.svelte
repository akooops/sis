<script>
    ...
    export let selectedQpRecord = null;

    let showCodeGenerator = false;

    function closeDrawer() {
        document.querySelector('[data-kt-drawer-dismiss="#qp_record_view_drawer"]')?.click();
    }
...
<div class="hidden kt-drawer kt-drawer-end card flex-col max-w-[90%] w-[450px] top-5 bottom-5 end-5 rounded-xl border border-border" data-kt-drawer="true" data-kt-drawer-container="body" id="qp_record_view_drawer">
    <div class="flex items-center justify-between gap-2.5 text-sm text-mono font-semibold px-5 py-2.5 border-b border-b-border">
        <div class="flex items-center gap-2">QP Record Details</div>
        ...
        {#if selectedQpRecord}
            <div class="space-y-4">
                <div class="flex items-center justify-between">
                    ...
                    <span class="text-sm font-medium">#{selectedQpRecord.id}</span>
                </div>

                <div class="flex items-center justify-between">
                    <span class="text-sm font-medium text-muted-foreground min-w-20">Code:</span>
                    {#if selectedQpRecord.code}
                        <span class="text-sm font-medium text-mono">{selectedQpRecord.code}</span>
                    {:else}
                ...
                <div class="flex items-center justify-between">
                    ...
                    <span class="kt-badge kt-badge-outline {getWarehouseStatusBadgeClass(selectedQpRecord.status)} text-xs font-medium capitalize">
                        {formatWarehouseStatus(selectedQpRecord.status)}
                    </span>
                </div>

                <div class="border-b border-border w-full"></div>

                {#if selectedQpRecord.w_h_good_receipt}
                    <div class="flex items-center justify-between gap-3">
                        <span class="text-sm font-medium text-muted-foreground min-w-20">WH Receipt:</span>
                        <span class="text-sm font-medium">{selectedQpRecord.w_h_good_receipt.code || `#${selectedQpRecord.w_h_good_receipt.id}`}</span>
                    </div>
                {/if}

                <div class="flex items-center justify-between gap-3">
                    <span class="text-sm font-medium text-muted-foreground min-w-20">Product:</span>
                    {#if selectedQpRecord.product}
                        <div class="flex items-center gap-3">
                            <img src={selectedQpRecord.product.image_url} alt={selectedQpRecord.product.name} class="w-[30px] h-[30px] rounded-lg object-cover" />
                            <span class="text-sm font-medium">{selectedQpRecord.product.name}</span>
                        </div>
                    {:else}
                        <span class="kt-badge kt-badge-outline kt-badge-secondary text-xs font-medium capitalize">N/A</span>
                    {/if}
                ...
                <div class="flex items-center justify-between">
                    ...
                    {#if formatQuantityWithUnit(selectedQpRecord.product, selectedQpRecord.quantity_approved)}
                        <span class="text-sm font-medium">{formatQuantityWithUnit(selectedQpRecord.product, selectedQpRecord.quantity_approved)}</span>
                    {:else}
                        <span class="kt-badge kt-badge-outline kt-badge-secondary text-xs font-medium capitalize">N/A</span>
                    {/if}
                </div>
                ...
                <div class="flex items-center justify-between">
                    ...
                    {#if formatQuantityWithUnit(selectedQpRecord.product, selectedQpRecord.quantity_released)}
                        <span class="text-sm font-medium">{formatQuantityWithUnit(selectedQpRecord.product, selectedQpRecord.quantity_released)}</span>
                    {:else}
                        <span class="kt-badge kt-badge-outline kt-badge-secondary text-xs font-medium capitalize">N/A</span>
                    {/if}
                </div>
                ...
                <div class="space-y-3">
                    <span class="text-sm font-semibold text-mono">QP Dates</span>
                    {#if selectedQpRecord.planned_qp_date}
                        <div class="flex items-center justify-between">
                            <span class="text-sm font-medium text-muted-foreground min-w-20">Planned:</span>
                            <span class="text-sm font-medium">{selectedQpRecord.planned_qp_date}</span>
                        </div>
                    {/if}
                    {#if selectedQpRecord.actual_qp_date}
                        <div class="flex items-center justify-between">
                            <span class="text-sm font-medium text-muted-foreground min-w-20">Actual:</span>
                            <span class="text-sm font-medium">{selectedQpRecord.actual_qp_date}</span>
                        </div>
                    {/if}
                </div>

                {#if selectedQpRecord.notes}
                    <div class="space-y-1">
                        <span class="text-sm font-medium text-muted-foreground">Notes:</span>
                        <p class="text-sm text-secondary-foreground">{selectedQpRecord.notes}</p>
                    </div>
                {/if}
                ...
                <div class="kt-card border border-border">
                    ...
                    <div class="kt-card-content p-4">
                        {#if selectedQpRecord.documents?.length}
                            <div class="space-y-2">
                                {#each selectedQpRecord.documents as document}
                ...
                <div class="flex items-center justify-between">
                    ...
                    <span class="text-sm font-medium">{formatTimeStamp(selectedQpRecord.created_at)}</span>
                </div>

                <div class="flex items-center justify-between">
                    <span class="text-sm font-medium text-muted-foreground min-w-20">Updated:</span>
                    <span class="text-sm font-medium">{formatTimeStamp(selectedQpRecord.updated_at)}</span>
                </div>
                ...
                {#if showCodeGenerator}
                    <BarcodeQRGenerator value={selectedQpRecord.id.toString()} title="QP Record ID Codes" showTitle={true} />
                {/if}
            </div>
        {/if}
    </div>
