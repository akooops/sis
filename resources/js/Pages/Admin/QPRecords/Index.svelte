<script>
    ...
    const breadcrumbs = [{ title: 'QP Records', url: route('web.admin.qp-records.index'), active: false }, { title: 'Index', url: route('web.admin.qp-records.index'), active: true }];
    const pageTitle = 'QP Records';

    let qpRecords = [], search = '', loading = true, perPage = 10, currentPage = 1, pagination = {};
    let filters = { sort_direction: 'desc', status: '', product_id: '', w_h_good_receipt_id: '' };
    let activeForm = null, activeRecord = null, selectedQpRecord = null;
    ...
    async function fetchRecords() {
        ...
        try {
            ...
            const res = await fetch(route('api.v1.admin.qp-records.index', q), { headers: { 'X-Requested-With': 'XMLHttpRequest' } });
            const data = await res.json();
            qpRecords = data.q_p_records || [];
    ...
    function openViewDrawer(record) {
        selectedQpRecord = record;
        document.querySelector('[data-kt-drawer-toggle="#qp_record_view_drawer"]')?.click();
    }
    ...
    async function handleCancel(record) {
        if (!confirm(`Cancel QP record "${record.code || `#${record.id}`}"?`)) return;
        const result = await putRecordAction('api.v1.admin.qp-records.cancel', { qpRecord: record.id }, null, 'QP record cancelled');
...
<MainLayout {breadcrumbs} {pageTitle}>
    <div class="grid gap-5 lg:gap-7.5">
        <div class="kt-card kt-card-grid min-w-full overflow-hidden">
            <div class="kt-card w-full border-0">
                    ...
                    <div class="kt-card-toolbar flex items-center justify-between w-full">
                        {#if !activeForm}
                            <div class="flex items-center gap-2">
                                <SearchBar bind:value={search} placeholder="Search QP records..." debounceMs={500} on:search={() => { currentPage = 1; fetchRecords(); }} />
                                <button type="button" class="kt-btn kt-btn-sm kt-btn-ghost" on:click={() => document.querySelector('[data-kt-drawer-toggle="#qp_records_filters_drawer"]')?.click()}><i class="fa-solid fa-filter"></i></button>
                                <ExportButton tableData={qpRecords} headers={[{ key: 'id', label: 'ID' }, { key: 'code', label: 'Code' }, { key: 'status', label: 'Status' }]} filename="qp-records" totalRecords={pagination?.total || 0} currentPerPage={perPage} {filters} />
                            </div>
                        {:else}
                ...
                {#if activeForm === 'edit'}
                    <div class="kt-card-content p-4" in:fly={{ x: '100%', duration: 750 }}><EditForm qpRecord={activeRecord} on:updated={handleDone} on:documentDeleted={fetchRecords} on:canceled={closeForm} /></div>
                {:else if activeForm === 'release'}
                    <div class="kt-card-content p-4" in:fly={{ x: '100%', duration: 750 }}><ReleaseForm qpRecord={activeRecord} on:released={handleDone} on:canceled={closeForm} /></div>
                {:else}
                            ...
                            <table class="kt-table kt-table-auto kt-table-border text-sm">
                                <thead>
                                    <tr>
                                        ...
                                        <th><span class="kt-table-col whitespace-nowrap capitalize">Released</span></th>
                                ...
                                <tbody>
                                    {#if loading}
                                        ...
                                        {/each}
                                    {:else if qpRecords.length === 0}
                                        <tr>
                                            <td colspan="6" class="p-10">
                                                <div class="flex flex-col items-center justify-center text-center">
                                                    <i class="fa-solid fa-box-open text-4xl text-muted-foreground mb-4"></i>
                                                    <h3 class="text-lg font-semibold text-mono mb-2">No results found</h3>
                                                    <p class="text-sm text-secondary-foreground">No QP records match your criteria.</p>
                                                </div>
                                            </td>
                                        </tr>
                                    {:else}
                                        {#each qpRecords as record}
                                        <tr class="hover:bg-muted cursor-pointer">
                                            ...
                                            <td on:click={() => handleRowClick(record)}>
                                                {#if fmt(record.product, record.quantity_released)}
                                                    <span class="text-sm font-medium">{fmt(record.product, record.quantity_released)}</span>
                                                {:else}
                                                    <span class="kt-badge kt-badge-outline kt-badge-secondary text-xs">N/A</span>
                                                {/if}
                                            </td>
                                                ...
                                                <div class="kt-menu flex-inline" data-kt-menu="true">
                                                    <div class="kt-menu-item" data-kt-menu-item-offset="0, 10px" data-kt-menu-item-placement="bottom-end" data-kt-menu-item-toggle="dropdown" data-kt-menu-item-trigger="click">
                                                        ...
                                                        <div class="kt-menu-dropdown kt-menu-default w-full max-w-[175px]">
                                                            ...
                                                            {#if hasPermission('qp-records.update') && record.can_be_updated}<div class="kt-menu-item"><button class="kt-menu-link" data-kt-menu-dismiss="true" on:click={() => openForm('edit', record)}><span class="kt-menu-icon"><i class="fa-solid fa-pen"></i></span><span class="kt-menu-title">Edit</span></button></div>{/if}
                                                            {#if hasPermission('qp-records.release') && record.can_be_released}<div class="kt-menu-item"><button class="kt-menu-link" data-kt-menu-dismiss="true" on:click={() => openForm('release', record)}><span class="kt-menu-icon"><i class="fa-solid fa-box-open"></i></span><span class="kt-menu-title">Release</span></button></div>{/if}
                                                            {#if hasPermission('qp-records.cancel') && record.can_be_cancelled}<div class="kt-menu-item"><button class="kt-menu-link text-destructive" data-kt-menu-dismiss="true" on:click={() => handleCancel(record)}><span class="kt-menu-icon"><i class="fa-solid fa-ban"></i></span><span class="kt-menu-title">Cancel</span></button></div>{/if}
                                                        </div>
                                                    </div>
                                                </div>
                                            </td>
    ...
    <button style="display:none" data-kt-drawer-toggle="#qp_records_filters_drawer"></button>
    <button style="display:none" data-kt-drawer-toggle="#qp_record_view_drawer"></button>
    <FiltersDrawer {filters} on:filtersChanged={(e) => { filters = e.detail; currentPage = 1; fetchRecords(); }} />
    <ViewDrawer {selectedQpRecord} />
</MainLayout>
