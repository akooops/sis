<script>
    ...
    $: if (qpRecord) {
        form = { planned_qp_date: qpRecord.planned_qp_date ?? '', notes: qpRecord.notes ?? '', documents: [] };
        existingDocuments = qpRecord.documents ? [...qpRecord.documents] : [];
    }
    $: canEdit = qpRecord?.can_be_updated !== false;
    ...
    async function handleSubmit() {
        ...
        try {
            ...
            const response = await fetch(route('api.v1.admin.qp-records.update', { qpRecord: qpRecord.id }), {
                method: 'POST',
            ...
            if (response.ok) { toast('QP record updated', 'success'); dispatch('updated'); }
...
<div class="space-y-6">
    <form on:submit|preventDefault={handleSubmit} class="space-y-5">
        ...
        <div class="kt-card border border-border">
            <div class="kt-card-header border-b border-border"><span class="text-sm font-semibold">QP Details</span></div>
            <div class="kt-card-content p-4 space-y-5">
                <div class="flex flex-col gap-2">
                    <label class="text-sm font-medium" for="planned-qp-date">Planned QP Date</label>
                    <Flatpickr id="planned-qp-date" bind:value={form.planned_qp_date} disabled={loading || !canEdit} />
                </div>
                <div class="flex flex-col gap-2">
                    <label class="text-sm font-medium" for="qp-notes">Notes</label>
                    <textarea id="qp-notes" class="kt-textarea min-h-[90px]" bind:value={form.notes} disabled={loading || !canEdit}></textarea>
                </div>
