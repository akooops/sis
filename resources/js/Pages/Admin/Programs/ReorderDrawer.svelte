<script>
    /**
     * Reorder drawer — drag the programs into the order they should appear in.
     *
     * A flat sortable list rather than a tree: streams and grades hang off a
     * program but are ordered on their own pages, so there is nothing to nest.
     ...
     * to be on screen would write positions 0-14 over programs that already hold
    ...
    /** Every program, in stored order — however many pages that takes. */
    async function loadAll() {
        const url = route('api.v1.admin.programs.index');
        const all = [];
        let page = 1;
        let lastPage = 1;
    ...
    $effect(() => {
        ...
        loadAll()
            ...
            .catch((e) => toast.error(e?.message ?? 'Could not load programs.'))
    ...
    async function save() {
        saving = true;
        try {
            await api.post(route('api.v1.admin.programs.reorder'), { ids: items.map((i) => i.id) });
...
<Drawer bind:open title="Reorder programs">
    <div class="flex flex-col gap-4">
        ...
        {:else if items.length === 0}
            <EmptyState icon="ki-filled ki-abstract-26" title="No programs" body="Add a program first." />
        {:else}
            ...
            <div class="flex flex-col gap-2">
                {#each items as item, index (item.id)}
                    ...
                    >
                        ...
                        <img src={item.thumbnail_url} alt="" class="size-8 shrink-0 rounded object-contain" />
