<script>
    ...
    /**
     * Every image this event has ever linked, as { id, url, name }. Seeded from
     ...
     */
    let imageRefs = $state(event?.images ? [...event.images] : []);
...
<form class="flex w-full flex-col gap-5" onsubmit={submit}>
    ...
    {#if !editing || activeTab === 'details'}
        <div class="flex flex-col gap-5">
            ...
            <Field label="Stylesheet URL" error={form.errors.css_url} hint="Optional external CSS applied to this event's content.">
                <Input bind:value={form.data.css_url} invalid={!!form.errors.css_url} placeholder="https://…" />
            </Field>

            <Field label="Custom CSS" error={form.errors.custom_css} hint="Inline CSS applied to this event's content.">
                <textarea
