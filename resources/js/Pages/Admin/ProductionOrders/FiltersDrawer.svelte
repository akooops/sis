<script>
    ...
    function closeDrawer() {
        document.querySelector('[data-kt-drawer-dismiss="#production_orders_filters_drawer"]')?.click();
    }
</script>

<div class="hidden kt-drawer kt-drawer-end card flex-col max-w-[90%] w-[450px] top-5 bottom-5 end-5 rounded-xl border border-border" data-kt-drawer="true" data-kt-drawer-container="body" id="production_orders_filters_drawer">
    <div class="flex items-center justify-between gap-2.5 text-sm text-mono font-semibold px-5 py-2.5 border-b border-b-border">
        <div class="flex items-center gap-2">Filter Production Orders</div>
    ...
    <div class="kt-card-content flex flex-col space-y-3 p-4 pt-2 kt-scrollable-y-auto">
        ...
        <div>
            <label class="text-sm font-semibold text-mono" for="production-order-product-filter">Product</label>
            <Select2
                bind:this={productSelectComponent}
                id="production-order-product-filter"
                ...
                ajax={{
                    ...
                    data: function (params) {
                        return { search: params.term, per_page: 10, production_order_eligible: 1 };
                    },
