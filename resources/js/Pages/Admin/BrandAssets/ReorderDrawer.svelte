<script>
    /**
     * Reorder drawer — drag one group's assets into the order they appear in.
     *
     * Order is per group: each numbers its assets from 0, so the drawer works on a
     * single group at a time and reordering a mixed list would write colliding
     * positions. The picker is seeded from the page's active group filter, so
     * drilling in from Asset groups lands ready to drag.
     *
    ...
    let { open = $bindable(false), groupId = null, onsaved } = $props();
    ...
    /** Every asset of one group, in stored order — however many pages that takes. */
    async function loadAll(id) {
        const url = route('api.v1.admin.brand-assets.index');
        const all = [];
        let page = 1;
        let lastPage = 1;

        do {
            const res = await api.get(url, { per_page: 100, sort: 'order', page, filter: { brand_asset_group_id: id } });
            all.push(...(res?.data ?? []));
    ...
    async function load(id) {
        // Token so switching group mid-fetch can't be clobbered by the older list.
        const my = ++loadToken;
        items = [];
        if (!id) return;
        ...
        try {
            ...
            toast.error(e?.message ?? 'Could not load assets.');
        } finally {
    ...
    async function save() {
        saving = true;
        try {
            await api.post(route('api.v1.admin.brand-assets.reorder'), { ids: items.map((i) => i.id) });
...
<Drawer bind:open title="Reorder assets">
    <div class="flex flex-col gap-4">
        <Field label="Group" hint="Assets are ordered within their group, one group at a time.">
            <Select
        ...
        {#if !selected}
            <EmptyState icon="ki-filled ki-folder" title="Pick a group" body="Choose an asset group to see the assets you can reorder." />
        {:else if loading}
            <div class="flex items-center justify-center gap-2 py-8 text-sm text-muted-foreground">
                <Spinner size="sm" /> Loading…
            </div>
        {:else if items.length === 0}
            <EmptyState icon="ki-filled ki-abstract-26" title="No assets" body="This group has no assets yet." />
        {:else}
