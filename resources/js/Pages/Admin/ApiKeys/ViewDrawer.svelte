<script>
    ...
    // Api Key data
    export let selectedApiKey = null;
    ...
    <div class="kt-card-content flex flex-col space-y-4 p-5 kt-scrollable-y-auto">
            ...
            <div class="space-y-4">
                <div class="space-y-3">
                    <div class="flex items-center justify-between">
                        ...
                        <span class="text-sm font-medium word-break">#{selectedApiKey.id}</span>
                    </div>
                    <div class="flex items-center justify-between">
                        <span class="text-sm font-medium text-muted-foreground min-w-20">Name:</span>
                        <span class="text-sm font-medium text-mono word-break">{selectedApiKey.name}</span>
                    </div>

                    <div class="flex items-center justify-between">
                        <span class="text-sm font-medium text-muted-foreground">Public Key:</span>
                        <code class="text-xs bg-muted px-2 py-1 rounded font-mono">{selectedApiKey.key}</code>
                    </div>

                    <div class="flex items-start justify-between">
                        <span class="text-sm font-medium text-muted-foreground min-w-20">Created:</span>
                        <div class="flex flex-col">
                            <span class="text-sm font-medium">{formatTimeStamp(selectedApiKey.created_at)}</span>
                        </div>
                    </div>
                    ...
                    <div class="flex items-start justify-between">
                        ...
                        <div class="flex flex-col">
                            <span class="text-sm font-medium">{formatTimeStamp(selectedApiKey.updated_at)}</span>
                        </div>
                    </div>
                </div>
                ...
                {#if showCodeGenerator}
                    <BarcodeQRGenerator 
                        value={selectedApiKey.id.toString()} 
                        title="Api Key ID Codes"
                        showTitle={true}
                    />
                {/if}
            </div>
        {:else}
            ...
            <div class="flex flex-col items-center justify-center text-center p-8">
                ...
                <h3 class="text-lg font-semibold text-mono mb-2">No Api Key Selected</h3>
                <p class="text-sm text-muted-foreground">
                    Select a api key to view its details.
                </p>
            </div>
        {/if}
    </div>
