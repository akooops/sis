<script>
    ...
    async function fetchPermissions() {
        ...
        try {
            // Load current role permissions with pagination
            const currentResponse = await fetch(route('api.v1.admin.role-permissions.index', { 
                role: role.id,
                page: currentPage,
                per_page: perPage,
                search: search,
            ...
            if (currentResponse.ok) {
                ...
                rolePermissions = currentData.role_permissions || [];
    ...
    async function handleDeleteRolePermission(rolePermission) {
        // Show confirmation dialog
        const confirmed = confirm('Are you sure you want to remove this permission from the role?\n\nThis action cannot be undone.');
        ...
        try {
            const response = await fetch(route('api.v1.admin.role-permissions.destroy', { rolePermission: rolePermission.id }), {
                method: 'DELETE',
                headers: {
                    'X-Requested-With': 'XMLHttpRequest',
                    'X-CSRF-TOKEN': document.querySelector('meta[name="csrf-token"]').getAttribute('content'),
            ...
            if (response.ok) {
                ...
                toast('Role permission removed successfully', 'success');
                fetchPermissions();
            } else {
                // Handle validation errors (422)
                if (response.status === 422 && data.errors) {
                    let errorMessage = 'Cannot remove role permission:\n';
                    ...
                    // Handle other errors
                    toast(data.message || 'An error occurred while removing the role permission.', 'error');
                }
            }
        } catch (error) {
...
<div class="hidden kt-drawer kt-drawer-end card flex-col max-w-[95%] w-[600px] top-5 bottom-5 end-5 rounded-xl border border-border overflow-hidden-x" data-kt-drawer="true" data-kt-drawer-container="body" id="role_permissions_drawer">
    <!-- Drawer Header -->
    <div class="flex items-center justify-between gap-2.5 text-sm text-mono font-semibold px-5 py-2.5 border-b border-b-border">
        <div class="flex items-center gap-2">
            Role Permissions:
            {#if role}
                <span class="text-muted-foreground word-break">{role.name}</span>
            {/if}
        </div>
            ...
            <div class="kt-card-toolbar flex items-center justify-between w-full">
                ...
                {:else}
                    <div class="flex items-center justify-between w-full">
                        ...
                        {#if hasPermission('role-permissions.store')}
                            <button
                                type="button"
                                    ...
                                    <td colspan="3" class="p-10">
                                        <div class="flex flex-col items-center justify-center text-center">
                                            ...
                                            <p class="text-sm text-secondary-foreground mb-4">
                                                {search ? 'No permissions match your search criteria.' : 'This role has no permissions assigned yet.'}
                                            </p>
                                        </div>
                                    </td>
                                </tr>
                            {:else}
                                ...
                                {#each rolePermissions as rolePermission}
                                    <tr class="hover:bg-muted cursor-pointer">
                                        <td>
                                            <a href={route('web.admin.permissions.index', { search: rolePermission.permission.id })}>        
                                                <span class="text-xs font-medium text-primary">#{rolePermission.permission.id}</span>
                                            </a>
                                        </td>
                                        <td>
                                            ...
                                            <div class="kt-menu flex-inline" data-kt-menu="true">
                                                <div class="kt-menu-item" data-kt-menu-item-offset="0, 10px" data-kt-menu-item-placement="bottom-end" data-kt-menu-item-placement-rtl="bottom-start" data-kt-menu-item-toggle="dropdown" data-kt-menu-item-trigger="click">
                                                    ...
                                                    <div class="kt-menu-dropdown kt-menu-default w-full max-w-[175px]">
                                                        {#if hasPermission('role-permissions.destroy')}
                                                            <div class="kt-menu-item">
                                                                <button class="kt-menu-link text-destructive" data-kt-menu-dismiss="true" on:click={() => handleDeleteRolePermission(rolePermission)}>
