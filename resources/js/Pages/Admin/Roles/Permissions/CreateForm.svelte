<script>
    ...
    async function handleSubmit() {
        ...
        try {
            ...
            const response = await fetch(route('api.v1.admin.role-permissions.store', { role: role.id }), {
                method: 'POST',
                headers: {
            ...
            if (response.ok) {
                ...
                toast('Permissions assigned successfully', 'success');
                dispatch('created');
            } else {
                ...
                if (data.errors) {
                    ...
                    // Handle other errors
                    console.error('Error assigning permissions:', data.message || 'Unknown error');
                    errors = { general: data.message || 'An error occurred while assigning permissions.' };
                }
            }
        } catch (error) {
    ...
    <form on:submit|preventDefault={handleSubmit} class="space-y-5">
        ...
        <div class="flex flex-col gap-2">
            <label class="text-sm font-medium text-mono" for="permission-select">
                Select Permissions <span class="text-destructive">*</span>
            </label>
            <Select2
