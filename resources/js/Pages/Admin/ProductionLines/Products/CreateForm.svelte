<script>
    ...
    async function handleSubmit() {
        ...
        try {
            ...
            const response = await fetch(route('api.v1.admin.line-products.store', { productionLine: productionLine.id }), {
                method: 'POST',
                headers: {
            ...
            if (response.ok) {
                ...
                toast('Line product added successfully', 'success');
                dispatch('created');
            } else if (data.errors) {
                errors = data.errors;
                ...
                errors = { general: data.message || 'An error occurred while adding the line product.' };
            }
        } catch (error) {
