<script>
    ...
    $: if (productionLine) {
        form = {
            ...
            production_site_id: productionLine.production_site_id || '',
            production_process_id: productionLine.production_process_id || '',
        };

        if (siteSelectComponent && productionLine.production_site_id) {
            siteSelectComponent.setValue(productionLine.production_site_id);
        }
    }

    $: if (productionLine && processSelectComponent && form.production_site_id === productionLine.production_site_id) {
        processSelectComponent.setValue(productionLine.production_process_id || '');
    }
    ...
    async function handleSubmit() {
        ...
        try {
            ...
            const response = await fetch(route('api.v1.admin.production-lines.update', productionLine.id), {
                method: 'POST',
                headers: {
            ...
            if (response.ok) {
                toast('Production line updated successfully', 'success');
                dispatch('updated');
            } else {
                if (data.errors) {
                    errors = data.errors;
                    ...
                    console.error('Error updating production line:', data.message || 'Unknown error');
                }
            }
        } catch (error) {
        ...
        <div class="kt-card border border-border">
            ...
            <div class="kt-card-content p-4 space-y-5">
                <div class="flex flex-col gap-2">
                    <label class="text-sm font-medium text-mono" for="edit-production-site-select">
                        Production Site <span class="text-destructive">*</span>
                    </label>
                    <Select2
                        bind:this={siteSelectComponent}
                        id="edit-production-site-select"
                ...
                <div class="flex flex-col gap-2">
                    <label class="text-sm font-medium text-mono" for="edit-production-process-select">
                        Production Process <span class="text-destructive">*</span>
                    </label>
                    {#key processSelectKey}
                        <Select2
                            bind:this={processSelectComponent}
                            id="edit-production-process-select"
                ...
                <div class="flex flex-col gap-2">
                    <label class="text-sm font-medium text-mono" for="edit-name">
                ...
                <div class="flex flex-col gap-2">
                    <label class="text-sm font-medium text-mono" for="edit-description">Description</label>
                    <textarea
                        id="edit-description"
