<script>
    ...
    function closeDrawer() {
        document.querySelector('[data-kt-drawer-dismiss="#qc_records_filters_drawer"]')?.click();
    }
</script>

<div class="hidden kt-drawer kt-drawer-end card flex-col max-w-[90%] w-[450px] top-5 bottom-5 end-5 rounded-xl border border-border" data-kt-drawer="true" data-kt-drawer-container="body" id="qc_records_filters_drawer">
    <div class="flex items-center justify-between gap-2.5 text-sm text-mono font-semibold px-5 py-2.5 border-b border-b-border">
        <div class="flex items-center gap-2">Filter QC Records</div>
    ...
    <div class="kt-card-content flex flex-col space-y-3 p-4 pt-2 kt-scrollable-y-auto">
        <div>
            ...
            <select bind:value={tempFilters.status} class="kt-select">
                ...
                <option value="submitted_to_qa">Submitted to QA</option>
                <option value="cancelled">Cancelled</option>
            </select>
        </div>

        <div>
            <label class="text-sm font-semibold text-mono" for="qc-product-filter">Product</label>
            <Select2
                bind:this={productSelectComponent}
                id="qc-product-filter"
        ...
        <div>
            <label class="text-sm font-semibold text-mono" for="qc-wh-filter">WH Good Receipt</label>
            <Select2
