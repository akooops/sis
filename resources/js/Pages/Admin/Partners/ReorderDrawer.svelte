<script>
    /**
     * Reorder drawer — drag the partners into the order they should appear in.
     *
     * A flat sortable list rather than a tree: partners have no parent/child, so
     * there is nothing to nest. Submits the whole list of ids, because the server
     * takes position-in-array as the order and leaves anything omitted alone.
     *
     ...
     * to be on screen would write positions 0-14 over partners that already hold
     * them elsewhere in the list. The API caps per_page at 100, hence the loop
     * rather than one big request; in practice one round-trip covers it.
     */
    ...
    /** Every partner, in stored order — however many pages that takes. */
    async function loadAll() {
        const url = route('api.v1.admin.partners.index');
        const all = [];
        let page = 1;
        let lastPage = 1;
    ...
    $effect(() => {
        ...
        loadAll()
            ...
            .catch((e) => toast.error(e?.message ?? 'Could not load partners.'))
    ...
    async function save() {
        saving = true;
        try {
            await api.post(route('api.v1.admin.partners.reorder'), { ids: items.map((i) => i.id) });
...
<Drawer bind:open title="Reorder partners">
    <div class="flex flex-col gap-4">
        ...
        {:else if items.length === 0}
            <EmptyState icon="ki-filled ki-abstract-26" title="No partners" body="Add a partner first." />
        {:else}
            ...
            <div class="flex flex-col gap-2">
                {#each items as item, index (item.id)}
                    ...
                    >
                        ...
                        <img src={item.logo_url} alt="" class="size-8 shrink-0 rounded object-contain" />
