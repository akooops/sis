...
@section('canonical', route('calendars'))
...
<section>
    <div class="container pb-14 pt-6">
        ...
        <div class="pt-6" data-aos="fade-up" data-aos-duration="2000">
            <x-panel id="calendars-panel" :title="getLanguageKeyLocalTranslation('calendars_page_collapse_title')">
                <div class="overflow-x-auto">
                    <table class="table">
                        <thead>
                            <tr>
                                <th class="w-[25px]">#</th>
                                <th>{{ getLanguageKeyLocalTranslation('calendars_page_table_header_calendar') }}</th>
                                <th>{{ getLanguageKeyLocalTranslation('calendars_page_table_header_start_date') }}</th>
                                <th>{{ getLanguageKeyLocalTranslation('calendars_page_table_header_end_date') }}</th>
                                <th>{{ getLanguageKeyLocalTranslation('calendars_page_table_header_option') }}</th>
                            </tr>
                        </thead>
                        ...
                        <tbody>
                            @foreach ($calendars as $key => $calendar)
                                <tr>
                                    ...
                                    <td>{{ $calendar->getLocalTranslation('title') }}</td>
                                    <td>{{ $calendar->starts_at->format('Y-m-d') }}</td>
                                    <td>{{ $calendar->ends_at->format('Y-m-d') }}</td>
                                    <td>
                                        <a href="{{ $calendar->calendarUrl }}" target="_blank" rel="noopener"
                                            class="btn btn-sm">
