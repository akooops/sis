<script>
    ...
    const breadcrumbs = [
        ...
        },
        {
            ...
            url: route('admin.facility-reservations.index', { facility: facility?.id }),
            active: true
        }
    ];
    ...
    async function fetchFacilityReservations() {
        loading = true;
        try {
            const response = await fetch(route('admin.facility-reservations.index', {
                facility: facility.id,
                page: currentPage,
                perPage: perPage,
                search: search,
                facility_time_slot_id: selectedTimeSlotId
            }), {
                headers: {
            ...
            facilityReservations = data.facilityReservations;
    ...
    // Delete reservation
    async function deleteFacilityReservation(facilityReservationId) {
        ...
        try {
            ...
            const response = await fetch(route('admin.facility-reservations.destroy', { facilityReservation: facilityReservationId }), {
                method: 'POST',
                headers: {
                    'X-Requested-With': 'XMLHttpRequest'
                },
            ...
            if (response.ok) {
                ...
                const errorMessage = errorData.message || 'Error deleting reservation. Please try again.';
        ...
        <div class="grid gap-5 lg:gap-7.5">
            ...
            <div class="flex flex-col lg:flex-row lg:items-center lg:justify-between gap-4">
                <div class="flex flex-col gap-1">
                    <h1 class="text-2xl font-bold text-mono">Reservations Management</h1>
                    <p class="text-sm text-secondary-foreground">
                        Manage reservations and appointments for "{facility?.name}"
                    </p>
                </div>
            </div>
                            ...
                            {:else}
                                ...
                                <div class="w-64">
                                    <Select2
                                        ...
                                        data={[]}
                                        ajax={{
                                            url: route('admin.facility-time-slots.index', facility.id),
                                            dataType: 'json',
                                            delay: 300,
                                            ...
                                            processResults: function(data) {
                                                return {
                                                    results: data.facilityTimeSlots.map(slot => ({
                                                        id: slot.id,
                        ...
                        <table class="kt-table kt-table-border table-fixed">
                            ...
                            <tbody>
                                {#if loading}
                                    ...
                                    {/each}
                                {:else if !facilityReservations || facilityReservations.length === 0}
                                    <!-- Empty state -->
                                    <tr>
                                        <td colspan="8" class="p-10">
                                            <div class="flex flex-col items-center justify-center text-center">
                                                ...
                                                <h3 class="text-lg font-semibold text-mono mb-2">No reservations found</h3>
                                                <p class="text-sm text-secondary-foreground mb-4">
                                                    {search || selectedTimeSlotId ? 'No reservations match your search criteria.' : 'Reservation requests from the facility mini-site will appear here.'}
                                                </p>
                                            </div>
                                        </td>
                                    </tr>
                                {:else}
                                    <!-- Actual data rows -->
                                    {#each facilityReservations as facilityReservation}
                                        <tr class="hover:bg-muted/50">
                                            <td>
                                                <input class="kt-checkbox kt-checkbox-sm" type="checkbox" value={facilityReservation.id}/>
                                            </td>
                                            <td>
                                                <span class="text-sm font-medium text-mono">#{facilityReservation.id}</span>
                                            </td>
                                            <td>
                                                <div class="flex flex-col gap-1">
                                                    <span class="text-sm font-medium text-mono hover:text-primary">
                                                        {facilityReservation.name}
                                                    </span>
                                                    <span class="text-xs text-secondary-foreground">
                                                        {facilityReservation.email}
                                                    </span>
                                                    <span class="text-xs text-secondary-foreground">
                                                        {facilityReservation.phone}
                                                    </span>
                                                </div>
                                            </td>
                                            <td>
                                                <span class="text-sm text-secondary-foreground">
                                                    {formatDateTime(facilityReservation.facility_time_slot?.starts_at)} - {formatDateTime(facilityReservation.facility_time_slot?.ends_at)}
                                                </span>
                                            </td>
                                            <td>
                                                <span class="kt-badge kt-badge-outline kt-badge-primary">
                                                    {facilityReservation.guests_count}
                                                </span>
                                            </td>
                                                ...
                                                <div class="kt-menu flex-inline" data-kt-menu="true">
                                                    <div class="kt-menu-item" data-kt-menu-item-offset="0, 10px" data-kt-menu-item-placement="bottom-end" data-kt-menu-item-placement-rtl="bottom-start" data-kt-menu-item-toggle="dropdown" data-kt-menu-item-trigger="click">
                                                        ...
                                                        <div class="kt-menu-dropdown kt-menu-default w-full max-w-[175px]" data-kt-menu-dismiss="true">
                                                            {#if hasPermission('admin.facility-reservations.show')}
                                                            <div class="kt-menu-item">
                                                                <a class="kt-menu-link" href={route('admin.facility-reservations.show', { facilityReservation: facilityReservation.id })}>
                                                            ...
                                                            {/if}
                                                            {#if hasPermission('admin.facility-reservations.destroy')}
                                                                <div class="kt-menu-separator"></div>
                                                                <div class="kt-menu-item">
                                                                    <button class="kt-menu-link" on:click={() => deleteFacilityReservation(facilityReservation.id)}>
                                                                        <span class="kt-menu-icon">
