<script>
    ...
    const breadcrumbs = [
        ...
        {
            title: 'Order',
            url: route('admin.visit-services.order-page'),
            active: true
        }
    ];
    
    const pageTitle = 'Order Visit Services';
    ...
    async function updateOrder() {
        ...
        try {
            const response = await fetch(route('admin.visit-services.order'), {
                method: 'POST',
                headers: {
            ...
            if (data.status === 'success') {
                ...
                KTToast.show({
                    ...
                    message: 'Visit Service order updated successfully!',
                    variant: "success",
            ...
            KTToast.show({
                ...
                message: error.message || 'Failed to update visit service order',
                variant: "error",
                position: "bottom-right",
            });
        } finally {
        ...
        <div class="grid gap-5 lg:gap-7.5">
            ...
            <div class="flex flex-col lg:flex-row lg:items-center lg:justify-between gap-4">
                <div class="flex flex-col gap-1">
                    <h1 class="text-2xl font-bold text-mono">Order Visit Services</h1>
                    <p class="text-sm text-secondary-foreground">
                        Drag and drop visit services to reorder them
                    </p>
                </div>
                    ...
                    <div class="w-full">
                                ...
                                {#each visitServices as visitService}
                                    <li class="list-group-item" data-id={visitService.id}>
                                        <div class="flex items-center">
                                            ...
                                            <div class="flex-1">
                                                <span class="font-medium text-mono">{visitService.name}</span>
                                                <small class="text-secondary-foreground ml-2">(ID: {visitService.id})</small>
                                            </div>
                                            {#if loading}
                                                <div class="ml-3">
                        ...
                        {:else}
                            <div class="text-center py-8">
                                ...
                                <p class="text-secondary-foreground">No visit services found to order.</p>
                                <a href="{route('admin.visit-services.create')}" class="kt-btn kt-btn-primary mt-4">
